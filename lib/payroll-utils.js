...
function normalizePayrollRules(raw = {}) {
  ...
  const lengthDays = Math.floor(
    toNumber(parsed.pay_period_length_days, DEFAULT_RULES.pay_period_length_days)
  );
  const safeLength =
    lengthDays >= 1 && lengthDays <= 31
      ? lengthDays
      : DEFAULT_RULES.pay_period_length_days;
  ...
  return {
    ...
    pay_period_anchor_date: normalizeDateOnly(parsed.pay_period_anchor_date),
