...
module.exports = function createEmployeeMediaUpload(rootDir = __dirname) {
  ...
  const photoAllowedMimes = new Set(Object.values(photoAllowedExts));
  const idAllowedMimes = new Set(Object.values(idAllowedExts));
  ...
  function isAllowedMime(m, allowed) {
    return allowed.has(m);
  }

  function isAllowedExt(ext, allowed) {
    return !!allowed[ext];
  }
  ...
  const upload = multer({
    ...
    fileFilter(req, file, cb) {
      ...
      const isId = file.fieldname === 'id_document';
      const allowedExts = isId ? idAllowedExts : photoAllowedExts;
      const allowedMimes = isId ? idAllowedMimes : photoAllowedMimes;
      ...
      if (
        ...
        !isAllowedMime(file.mimetype, allowedMimes)
      ) {
