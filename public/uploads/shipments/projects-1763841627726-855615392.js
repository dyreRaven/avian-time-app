...
async function loadProjectsTable() {
  ...
  try {
    ...
    if (!projects.length) {
      tbody.innerHTML = `<tr><td colspan="3">(no ${projectsListStatus} projects)</td></tr>`;
      return;
    }
    ...
    projects.forEach(p => {
      ...
      tr.innerHTML = `
        <td>${projectName}${statusLabel}</td>
        <td>${customerName}</td>
        <td>${isActive ? '' : 'Inactive'}</td>
      `;

      // 🔗 open the project modal on click
