...
- 2026-02-23: Added kiosk punch soft rate limiting per device + employee for live punch attempts (`429 punch_rate_limited` with retry seconds). Offline queued punches bypass the throttle, and kiosk offline sync now treats 429 as retryable (keeps queue item and retries later) instead of dropping it as a hard error.
- 2026-02-23: QuickBooks onboarding callback resume now opens the QuickBooks setup modal directly (sync step) before rendering the Finish Setup checklist, eliminating the onboarding flash/bounce on return from Intuit; modal and step transitions were also smoothed for less jarring handoffs (with reduced-motion fallback).
- 2026-02-23: Offline punch sync now enforces a max queued age (`offline_punch_max_age_days`, default 14 days); stale queued punches are rejected with `offline_punch_too_old` and kiosk sync status shows a clear admin warning.
- 2026-02-23: QuickBooks payroll check lines now set `BillableStatus=Billable` for all non-reimbursement lines and `BillableStatus=NotBillable` for reimbursement lines; non-reimbursement lines without a linked QuickBooks project now fail with a clear billable-project error.
- 2026-02-23: Payroll Run Review now opens as a dedicated popup window with click-out/Escape close behavior, failed/success check sections stacked vertically, and a clear `Completed successfully` message when no failed checks are returned.
- 2026-02-23: Added Payroll `Run Status` management for stuck create-check runs: 409 in-progress errors now include active run + lock context and automatically open a Run Status modal with refresh, open review, and cancel/clear-lock actions (`GET /api/payroll/active-run`, `POST /api/payroll/active-run/cancel`).
- 2026-02-23: Payroll create-checks no longer hard-block standard runs for overlapping date ranges; overlapping periods are allowed, and only payroll-approved unpaid entries are eligible so previously paid entries are not sent again.
- 2026-02-23: Time exceptions now detect overlapping punch windows for the same employee and surface them in admin review as `Overlapping punch window`.
- 2026-02-23: Payroll create/retry now returns a clear blocking error when there are zero eligible entries (`No payroll-approved unpaid entries are eligible... No checks were sent.`), so empty sends do not appear as completed runs.
- 2026-02-23: Fixed payroll check creation crash caused by an undefined `allowNameOnChecksSync` reference in QuickBooks check creation; Create Checks now proceeds without that runtime exception.
- 2026-02-23: Notification Preferences now support a per-org email destination override for non-superadmins (`notification_email` on membership); when set, email alerts deliver to that override, otherwise they fall back to login email, and super admins remain locked to login-email delivery.
- 2026-02-23: Payroll create/retry request failures now show explicit `Create checks failed` / `Retry checks failed` messages (with HTTP status when available) plus the underlying server error, replacing the vague “problem contacting the server” wording.
- 2026-02-23: Payroll preflight/create-check confirmations now use in-app styled modals (instead of browser confirms), including `Create/Retry Checks`, `Re-run Preflight` on snapshot changes, and the “checks may fail” decision (`Create/Retry Ready Checks` vs `Cancel and Back`); modal pay-period dates are now rendered as `MM/DD/YYYY`, and choosing `Cancel and Back` cancels the flow before any QuickBooks create call.
- 2026-02-23: Payroll create/retry confirmation dialogs no longer repeat advisory pending-approval or field-review warnings; those advisories remain surfaced in payroll summary/preflight context instead of the create-check confirm popup.
